...
public class IncomeEntity {
    ...
    @ManyToOne(fetch = FetchType.LAZY) // Many incomes can belong to one category, and the category data will be loaded lazily (only when accessed)
    @JoinColumn(name = "category_id", nullable = false) // Specifies the foreign key column name and makes it non-nullable
    private CategoryEntity category;

    @ManyToOne(fetch = FetchType.LAZY) // Many incomes can belong to one profile, and the profile data will be loaded lazily (only when accessed)
